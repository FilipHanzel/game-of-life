...
class Game:
    __slots__ = ("map", "_np_map_buffer", "_neighbors")

    def __init__(self, size: int = 100):
        self.map = cp.zeros((size, size), dtype=cp.int8)
        # Buffer needed to get data from gpu before using it with pygame
        self._np_map_buffer = np.empty((size, size), dtype=np.int8)
        self._neighbors = cp.zeros(self.map.shape, dtype=cp.int8)
    ...
    def randomize(self):
        self.map = cp.random.randint(0, 2, size=self.map.shape, dtype=cp.int8)

    def get_map_from_gpu(self):
        self.map.get(out=self._np_map_buffer)
        return self._np_map_buffer
...
if __name__ == "__main__":
    ...
    while not end:
        ...
        game_map = game.get_map_from_gpu()

        screen_buffer[...] = game_map[:, None, :, None]
